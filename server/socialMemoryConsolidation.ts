...
Return only minified JSON matching the schema with zero to three merge actions. An action is allowed only when all cited memories describe the same recollection, or one is wholly subsumed by another without losing a meaningful detail. Similar topic, person, mood or channel is not enough. When uncertain, return {"actions":[]}.
