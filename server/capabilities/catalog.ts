...
export interface CapabilityArgumentContract {
  ...
  /**
   * Catalog-owned provider defaults for an already selected action. Applying
   * one may complete wire compatibility but can never select a capability or
   * synthesize its semantic subject.
   */
  defaultValues?: Readonly<Partial<Record<CapabilityArgumentField, string>>>;
...
const capabilityDefinitions = {
  ...
  market_snapshot: {
    ...
    routingGuidance: {
      primary: `use only for the latest reported numeric level and previous-close change of one supported headline equity index, or a bounded current overview of one registered market basket. Put exactly one canonical target ID in l and keep q/u/m/z/k null. Resolve common multilingual market wording semantically to the intended registered target, but never invent an ID or equate the Dow Jones Industrial Average with the broader Dow Jones U.S. Index. GLOBAL_MAJOR is the bounded major-world-index overview; a contextual follow-up asking how the rest of the world or other world markets performed after one index move maps to GLOBAL_MAJOR when it asks for current performance and does not ask for news or causes. Regional basket IDs are bounded overviews, not every exchange or security in that region. Before selecting this action, verify that returning the registered index/basket's own level, previous-close change or bounded basket overview would answer the complete request. When an individual company/security is described as a member, component or example inside an index/basket, that individual entity remains the subject and the containing index is only context; its valuation, condition, prospects or other requested information cannot be answered by returning the container's level; never replace the member with its container. A normal request to check one of these values is an execution question or request, not a capability-availability question. Individual equities, market news, history, causes, forecasts, advice and analysis remain web_search. Registered targets: ${marketTargetRoutingMap}.`,
      verifier: `market_snapshot requires exactly one registered canonical exact-index or fixed-basket ID in l with q/u/m/z/k null. Use it only for latest reported index levels and previous-close changes, including a bounded global or regional overview when the matching basket is explicitly or contextually intended. A follow-up asking how the rest of the world or other world markets performed after one supplied index move uses GLOBAL_MAJOR unless it asks for news or causes. Resolve wording semantically across languages; never invent an ID, map a company/security to an index, or treat DJIA as the broader DJUS index. Before selecting this action, verify that returning the registered index/basket's own level, previous-close change or bounded basket overview would answer the complete request. When an individual company/security is described as a member, component or example inside an index/basket, that individual entity remains the subject and the containing index is only context; its valuation, condition, prospects or other requested information cannot be answered by returning the container's level; never replace the member with its container. Keep individual equities, news, historical questions, causal explanations, forecasts, advice and analysis on web_search, and keep none if the target cannot be resolved safely. Registered targets: ${marketTargetRoutingMap}.`,
    },
  ...
  local_datetime: {
    ...
    arguments: {
      required: ["z", "k", "l"],
      allowed: ["z", "k", "l"],
      // A combined date-and-time answer is the narrowest safe superset when a
      // structured-output engine selects this capability and resolves its
      // zone/label but omits only the requested projection kind.
      defaultValues: { k: "current_datetime" },
    },
    routingGuidance: {
