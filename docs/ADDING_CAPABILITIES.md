...
## Example: future market data versus market news

“What is moving the market today?” or “find today's news about the car sector” is broad discovery. It should use `web_search` in `news` mode, followed by bounded page expansion. A market adapter should not take over merely because the room is `#stock-market` or the message contains a ticker-like token.

A dedicated `market_snapshot` adapter now handles one narrower request: the latest provider-reported level and session change for the fixed supported headline-index symbols `OMXS30` and `DJUS`. The adapter uses only fixed public provider endpoints, validates the numeric fields and absolute observation timestamp, filters the packet to the exact requested symbol and attaches the provider source. Because the provider does not expose a trustworthy market-open state, its scene contract must say “latest reported” with the dated update rather than asserting that the market is open or that the change belongs to the current calendar day. Unknown, missing or ambiguous symbols fail closed.

That adapter must not expand into company news, analyst commentary, personalized advice or broad market explanation. Those remain `web_search` or ordinary conversation. If `market_snapshot` is unavailable when the turn is first classified, the router may select `web_search` for a safely answerable general external request. If a trusted `market_snapshot` invocation is selected and its provider then fails, that invocation returns `failed_temporary`; the registry does not retry it as `web_search`.
