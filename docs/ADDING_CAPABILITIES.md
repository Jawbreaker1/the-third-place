...
A dedicated `market_snapshot` adapter could be justified for a narrower request such as “what are the current price, timestamp, currency and day change for instrument X?”—but only if the server has a fixed, authorized provider that returns structured live fields. Its catalog entry would require an unambiguous provider instrument identifier and declare price, currency, market timestamp, absolute/percentage change and market-state fields. The provider would validate them exactly, attach the licensed source/attribution required by its terms and reject stale or ambiguous instruments.
