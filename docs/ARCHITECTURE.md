...
Rare autonomous research is a separate director-owned path enabled only when both research and `AUTONOMOUS_RESEARCH_ENABLED` are active. It can run only for a fresh ambient thread with a human online, an Admin-enabled global and effective room **AI-posted links** policy, sufficient human quiet, an empty active-provider queue, two free publication slots, no voice room, the policy's probability gate, rolling in-process 24-hour success cap and global/per-channel cooldowns. The 0–100 control maps to bounded chance, quiet time, cooldown and capacity; zero is the unambiguous kill switch. Profiles supply bounded queries and discussion angles, never URLs. A search result not already shown in recent room history must then survive the DNS-pinned exact `PageReader`; failure is silent and enters bounded retry backoff. One research-capable lead and one distinct responder discuss the same `S1` detail. In server-card mode the model receives title/body but no URL, visible model-written URLs are mechanically rejected, and the director attaches the exact validated source/card. AI publication and metadata updates cannot schedule another lookup.
