...
Fresh information is exposed through a typed capability catalog. The current built-ins are `web_search`, `read_url`, `weather_forecast` and `local_datetime`: search exists only when `RESEARCH_ENABLED=true`; exact reading exists only when `LINK_READER_ENABLED=true` and the server supplied an opaque candidate reference; weather uses a fixed structured forecast provider for a bounded named location; and local date/time uses the server clock plus a validated IANA zone. The one-pass multilingual turn analysis—not a verb list or room-specific regex—may select at most one available capability with high confidence. General external discovery defaults to `web_search`; an exact supplied target remains `read_url`, and a narrower capability is selected only for its registered semantic scope with all required arguments resolved. Timeless opinions, negated requests and ordinary banter stay local. Voice advertises only `local_datetime`.
