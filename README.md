...
Full-page reading remains distinct from metadata previews. For explicit requests, the server collects a bounded set of links from the current human burst, its reply target and that same guest's recent room messages, then gives Gemma only opaque references such as `U1`—never authority to invent or rewrite a URL. The multilingual turn analysis decides whether the guest asked to read one, negated a request, referred to an earlier link or instead requested a web search. Exact wording is not classified with Swedish/English verb lists.
